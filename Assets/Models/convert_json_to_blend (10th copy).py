...
def create_truncated_circle_quadrant_ceiling(x, y, radius, collection, quadrant=1, ceiling_material=None, fixed_height=0.5):
    """
    Creates a truncated pyramid ceiling for a circle quadrant with inward-facing normals.
    ...
    - fixed_height: Fixed height for the top of the pyramid.
    """
    segment_length = 1.0
    arc_length = (math.pi / 2) * radius
    segments = max(1, int(arc_length / segment_length))
    ...
    base_verts = [(x, y, 0)]  # Center point at the base
    base_verts += [
    ...
    # Create top vertices (scaled inward and fixed height)
    top_verts = [(x, y, fixed_height)]  # Center point at the top
    top_verts += [
    ...
    # Create faces connecting the base and top
    faces = []
    base_offset = 0
    top_offset = len(base_verts)
    for i in range(1, len(base_verts) - 1):
        # Triangular faces for the base and top
        faces.append((base_offset, i, i + 1))  # Base triangle
        faces.append((top_offset, top_offset + i, top_offset + i + 1))  # Top triangle
        
        # Quadrilateral faces for the sides
        faces.append((i, i + 1, top_offset + i + 1, top_offset + i))
...
def add_rotunda_ceiling(rect, collection, ceiling_material):
    ...
    radius = (min(w, h) - 1) / 2
    obj = create_truncated_circle_quadrant_ceiling(x + w - (h / 2 - 0.5), y + h - (h / 2 - 0.5), radius, collection, quadrant=1, ceiling_material=ceiling_material, fixed_height=fixed_height)
    objs.append(obj)
    obj = create_truncated_circle_quadrant_ceiling(x + (h / 2 - 0.5), y + h - (h / 2 - 0.5), radius, collection, quadrant=2, ceiling_material=ceiling_material, fixed_height=fixed_height)
    objs.append(obj)
    obj = create_truncated_circle_quadrant_ceiling(x + (h / 2 - 0.5), y + (h / 2 - 0.5), radius, collection, quadrant=3, ceiling_material=ceiling_material, fixed_height=fixed_height)
    objs.append(obj)
    obj = create_truncated_circle_quadrant_ceiling(x + w - (h / 2 - 0.5), y + (h / 2 - 0.5), radius, collection, quadrant=4, ceiling_material=ceiling_material, fixed_height=fixed_height)
    objs.append(obj)

    # Cover the remaining part with a plus-shaped ceiling
    plus_objs = create_rotunda_plus_ceiling(x, y, w, h, ceiling_material, fixed_height)
