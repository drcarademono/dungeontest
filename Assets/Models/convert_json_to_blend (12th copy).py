...
def create_truncated_circle_quadrant_ceiling(x, y, radius, collection, quadrant=1, ceiling_material=None, room_ceiling_height=2.5, fixed_height=0.5):
    """
    Creates a truncated pyramid ceiling for a circle quadrant with inward-facing normals and reversed UV orientation.
    ...
    for i in range(1, len(base_verts) - 1):
        # Top triangle faces (reversed order)
        faces.append((top_offset + i + 1, top_offset + i, top_offset))  # Top triangle
        
        # Quadrilateral faces for the sides (reversed order)
        faces.append((top_offset + i + 1, i + 1, i, top_offset + i))
