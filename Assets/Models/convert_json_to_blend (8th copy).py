...
def create_truncated_pyramid_ceiling(x, y, w, h, ceiling_height, ceiling_material, top_scale=0.5):
    """
    Creates a truncated pyramid ceiling with the base fitting the room's walls and the top scaled.
    Normals are defined to face inward directly.
    """
    ...
    base_verts = [
        (x, y, ceiling_height),               # Bottom-left
        (x + w, y, ceiling_height),           # Bottom-right
        (x + w, y + h, ceiling_height),       # Top-right
        (x, y + h, ceiling_height)            # Top-left
    ]
    
    # Top vertices (scaled down from base)
    top_verts = [
        (x + w * (1 - top_scale) / 2, y + h * (1 - top_scale) / 2, ceiling_height + ceiling_height * top_scale),  # Bottom-left
        (x + w * (1 + top_scale) / 2, y + h * (1 - top_scale) / 2, ceiling_height + ceiling_height * top_scale),  # Bottom-right
        (x + w * (1 + top_scale) / 2, y + h * (1 + top_scale) / 2, ceiling_height + ceiling_height * top_scale),  # Top-right
        (x + w * (1 - top_scale) / 2, y + h * (1 + top_scale) / 2, ceiling_height + ceiling_height * top_scale)   # Top-left
    ]
    ...
    faces = [
        ...
        (7, 6, 5, 4)   # Top face (clockwise for inward normals)
    ]
...
def create_rotunda_plus_ceiling(x, y, w, h, ceiling_material, ceiling_height):
    ...
    # Create horizontal part of the plus
    for i in range(w):
        bpy.ops.mesh.primitive_plane_add(size=1, enter_editmode=False, align='WORLD', location=(x + i + 0.5, center_y, ceiling_height))
        plane = bpy.context.active_object
        objs.append(plane)
        add_uvs(plane, 1, 1)
        plane.data.materials.append(ceiling_material)

    # Create vertical part of the plus
    for j in range(h):
        bpy.ops.mesh.primitive_plane_add(size=1, enter_editmode=False, align='WORLD', location=(center_x, y + j + 0.5, ceiling_height))
        plane = bpy.context.active_object
        objs.append(plane)
        add_uvs(plane, 1, 1)
        plane.data.materials.append(ceiling_material)
...
def add_rotunda_ceiling(rect, collection, ceiling_material):
    ...
    objs = []
    # Create circle quadrant ceilings
    obj = create_circle_quadrant_ceiling(x + w - (h / 2 - 0.5), y + h - (h / 2 - 0.5), (min(w, h) - 1) / 2, collection, quadrant=1, ceiling_material=ceiling_material, ceiling_height=ceiling_height)
    objs.append(obj)
    obj = create_circle_quadrant_ceiling(x + (h / 2 - 0.5), y + h - (h / 2 - 0.5), (min(w, h) - 1) / 2, collection, quadrant=2, ceiling_material=ceiling_material, ceiling_height=ceiling_height)
    objs.append(obj)
    obj = create_circle_quadrant_ceiling(x + (h / 2 - 0.5), y + (h / 2 - 0.5), (min(w, h) - 1) / 2, collection, quadrant=3, ceiling_material=ceiling_material, ceiling_height=ceiling_height)
    objs.append(obj)
    obj = create_circle_quadrant_ceiling(x + w - (h / 2 - 0.5), y + (h / 2 - 0.5), (min(w, h) - 1) / 2, collection, quadrant=4, ceiling_material=ceiling_material, ceiling_height=ceiling_height)
    objs.append(obj)

    # Covering the remaining part with a plus-shaped ceiling
    plus_objs = create_rotunda_plus_ceiling(x, y, w, h, ceiling_material, ceiling_height)
    objs.extend(plus_objs)

    # Ensure all parts are selected and their normals are flipped
    for obj in objs:
...
def process_json_file(json_file):
    ...
    for rect in data['rects']:
        ...
        if 'rotunda' in rect and rect['rotunda']:
            ...
            ceiling_objs.append(add_ceiling_to_room(rect, collection, ceiling_material))
