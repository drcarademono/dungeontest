...
def process_json_file(json_file):
    ...
    for rect in data['rects']:
        ...
        if 'rotunda' in rect and rect['rotunda']:
            ...
            if 'walls' in rect:
                for wall in rect['walls']:
                    level = wall.get('level', 0)  # Use level 0 if 'level' is not specified
                    objects_to_merge.append(create_wall(wall['x'], wall['y'], wall['dir']['x'], wall['dir']['y'], rect['x'], rect['y'], rect['w'], rect['h'], wall_material=wall_material, level=level))
        else:
